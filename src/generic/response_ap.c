...
int apHeaderHandler(Tcl_Interp *interp, ResponseObj *responseObj,
   ...
   * sanity
   * ----------------------------------------------------------------------- */
  if( (interp == NULL) || (responseObj == NULL) ) return TCL_ERROR;
...
#ifndef APACHE2
	      r->content_type = ap_pstrdup(r->pool,Tcl_GetString(lobjv[i]));
	    } else {
	      ap_table_setn(r->headers_out,
			    ap_pstrdup(r->pool, key),
			    ap_pstrdup(r->pool, Tcl_GetString(lobjv[i])));
#else /* APACHE2 */
	      r->content_type = (char *)apr_pstrdup(r->pool, Tcl_GetString(lobjv[i]));
	    } else {
	      apr_table_setn(r->headers_out,
			    (char *)apr_pstrdup(r->pool, key),
			    (char *)apr_pstrdup(r->pool, Tcl_GetString(lobjv[i])));
#endif /* APACHE2 */
	    }
	  }
	}

...
int Web_Dispatch(ClientData clientData, 
  ...
  if( query_string == NULL ) {
     ...
     * --------------------------------------------------------------------- */
    query_string = paramListGetObjectByString(interp,requestData->request,
					      "QUERY_STRING");
  }
  ...
  if( query_string != NULL ) {
    ...
    if( Tcl_GetCharLength(query_string) > 0 ) {
      ...
      if( parseQueryString(requestData,interp,query_string) == TCL_ERROR )
	return TCL_ERROR;
    }
  }
...
/*         } */
	...
	case 2:
	   ...
	   * -postdata channel
	   * --------------------------------------------------------------- */
	  parsePostData(interp,objv[idx1+1],NULL,NULL,requestData);
	  break;
	case 3:
	  /* ------------------------------------------------------------------
	  ...
	  parsePostData(interp,objv[idx1+1],objv[idx1+2],NULL,requestData);
	  break;
	case 4:
	  /* ------------------------------------------------------------------
	   ...
	   * --------------------------------------------------------------- */
	  /* log is handled by parsePostData */
	  parsePostData(interp,objv[idx1+1],objv[idx1+2],objv[idx1+3],
			requestData);
	  break;
	default:
    ...
    content_type = paramListGetObjectByString(interp,requestData->request,
                                  "CONTENT_TYPE");
    content_length = paramListGetObjectByString(interp,requestData->request,
                                  "CONTENT_LENGTH");
   ...
   * ======================================================================= */
  {
    ...
    if( cmdName != NULL ) {
       ...
       * ------------------------------------------------------------------- */
      if( Tcl_GetCharLength(cmdName) < 1 ) return TCL_OK;
     ...
     * get command code
     * --------------------------------------------------------------------- */
    cmdCode = (Tcl_Obj *)getFromHashTable(requestData->cmdList,cmdNameStr);

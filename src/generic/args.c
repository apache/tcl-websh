...
int argOptionType(Tcl_Obj* in) {
  ...
  if( tmp[0] == '-') {
    ...
    if( tmpLen > 1 ) {
      if( tmp[1] != '-') {
	if (isdigit(tmp[1])) return OPTION_TYPE_NUMBER;
...
int argPosParam(char **params, char *key) {
  ...
  if( (key == NULL) || (params == NULL) ) return -1;

  while( *intParams != NULL ) {

    if( strcmp(*intParams,key) == 0) return pos;
...
int argIndexOfFirstArg(int objc, Tcl_Obj *CONST objv[], 
  ...
  if(objc < 2 ) return objc;
  if(objv == NULL) return objc;

  if( argOptionType(objv[1]) == OPTION_TYPE_NONE ) return 1;
  ...
  while( pos < objc ) {
	...
	break;
      case OPTION_TYPE_PARAM:
         ...
         */
	if( (pidx = argPosParam(params,Tcl_GetString(objv[pos]))) != -1 ) {
	  if( Nparams != NULL ) pos += Nparams[pidx];
          else                  pos++;
	}
        pos++;
        break;
...
int argHasOnlyAccepted(int objc, Tcl_Obj *CONST objv[], char *params[], 
  ...
  if( scanc < 0 ) scanc = objc;
  if( scanc > objc ) scanc = objc;

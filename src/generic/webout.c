	     ...
	     int objc, Tcl_Obj *CONST objv[])
{
   ...
   * 0         1     2
   * ----------------------------------------------------------------------- */
  WebAssertObjc((objc < 2) || (objc > 3), 1, "?channel|#globalvar? extendedstring");
...
int Web_Response(ClientData clientData, Tcl_Interp *interp,
  ...
  */
  ...
  static char *params[] = {"-sendheader",
			   ...
			   "-resetall",
			   NULL};
  enum params {SENDHEADER,SELECT,BYTESSENT,HTTPRESPONSE,RESET,RESETALL};
  ...
  outData = (OutData *)clientData;
  WebAssertData(interp,outData->responseObjHash,"web::response",TCL_ERROR);
  ...
  res = paramGet((ParamList *)responseObj->headers,
		 interp,objc,objv, 1);
  ...
  if (res == TCL_CONTINUE) {
	...
	if (isDefaultResponseObj(tname))
	  ...
	  responseObj = getResponseObj(interp, outData, tname);
        ...
        if(responseObj == NULL) {
	  Tcl_SetResult(interp, "could not reset response object", TCL_STATIC);
	  return TCL_ERROR;
	}
	...
	if (objc == 3)
	  ...
	  if( Tcl_GetBooleanFromObj(interp, objv[2],
				    &(responseObj->sendHeader)) == TCL_ERROR ) {
...
/*         fprintf(stderr,"DBG -select called\n"); fflush(stderr); */
	...
	/* we have to find the new channel */
        responseObj = getResponseObj(interp,outData,Tcl_GetString(objv[2]));
	if (responseObj == NULL) {
	  Tcl_ResetResult(interp);
	  Tcl_AppendResult(interp, "invalid response object \"",  Tcl_GetString(objv[2]), "\"", (char *) NULL);
	  return TCL_ERROR;
	}
	outData->defaultResponseObj = responseObj;
	Tcl_ResetResult(interp); /* empty string */
	...
	break;
      ...
      case BYTESSENT:
	WebAssertObjc(objc != 2 , 2, NULL);
	Tcl_SetObjResult(interp,Tcl_NewLongObj(responseObj->bytesSent));
	return TCL_OK;
	...
	if (objc == 3) {
	  ...
	  if (len == 0)
	    ...
	    if ( !strcmp("default", response) ) {
	      responseObj->httpresponse = Tcl_NewStringObj(HTTP_RESPONSE, -1);
	    } else {
	      responseObj->httpresponse = Tcl_DuplicateObj( objv[2] );
	    }

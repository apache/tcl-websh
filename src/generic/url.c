...
int resetUrlData(Tcl_Interp *interp, UrlData *urlData) {
  ...
  if( (interp == NULL) || (urlData == NULL) ) return TCL_ERROR;
...
int parseUrlFormat(Tcl_Interp *interp, Tcl_Obj *list) {
  ...
  if( Tcl_ListObjGetElements(interp,list,&objc,&objv) == TCL_ERROR ) {
    LOG_MSG(interp,WRITE_LOG | SET_RESULT,
            __FILE__,__LINE__,
            "web::cmdurl -urlformat",WEBLOG_ERROR,
            "cannot convert \"",Tcl_GetString(list),
            "\" to list",NULL);
    return 0;
  }
 ...
 * ------------------------------------------------------------------------- */
Tcl_Obj *mergeLists(Tcl_Interp *interp, Tcl_Obj *cmdlineP, Tcl_Obj *staticP) {
  ...
  if( (staticP == NULL) || (cmdlineP == NULL) ) return NULL;
...
Tcl_Obj *createQueryList(Tcl_Interp *interp, Tcl_Obj *cmd, Tcl_Obj *plist, 
  ...
  if( plist != NULL )
    if( Tcl_ListObjAppendList(interp,qStr,plist) == TCL_ERROR ) errCnt++;
  ...
  if( Tcl_ListObjAppendList(interp,qStr,tmp) == TCL_ERROR ) errCnt++;
      ...
      if( urlData->requestData != NULL )
        if( urlData->requestData->cmdTag != NULL )
          if( Tcl_ListObjAppendElement(interp,qStr,
              ...
              == TCL_ERROR ) errCnt++;
      if( errCnt < 1 ) 
        if( Tcl_ListObjAppendElement(interp,qStr,cmd) == TCL_ERROR ) errCnt++;
    }
  }
    ...
    if( urlData->requestData != NULL )
      if( urlData->requestData->timeTag != NULL )
        if( Tcl_ListObjAppendElement(interp,qStr,
            ...
            == TCL_ERROR ) errCnt++;
    if( errCnt < 1 ) 
      if( Tcl_ListObjAppendElement(interp,qStr,Tcl_NewLongObj(time(NULL))) == \
          TCL_ERROR ) errCnt++;
  }
...
int Web_CmdUrl(ClientData clientData, 
  ...
  if( (urlFmt = argValueOfKey(objc,objv,params[URLFORMAT])) != NULL ) {
    ...
    if( urlformat == 0 ) return TCL_ERROR;
  }
  ...
  if ((urlformat & WEB_URL_WITH_QUERYSTRING) != 0) {
	 ...
	 * ................................................................ */
	if( (plistLen = tclGetListLength(interp,objv[iCurArg])) == -1 )
	  return TCL_ERROR;
	if( (plistLen % 2) != 0 ) {
	  LOG_MSG(interp,WRITE_LOG | SET_RESULT,__FILE__,__LINE__,
		  "web::cmdurl",WEBLOG_INFO,
	...
	qStrList = createQueryList(interp,cmd,objv[iCurArg],urlData,flag);
	break;
      default:
	/* ................................................................
	...
	plist = Tcl_NewObj();
	if( plist == NULL ) return TCL_ERROR;
	for( i = iCurArg; i < objc; i += 2) {
	  if( Tcl_ListObjAppendElement(interp,plist,objv[i]) == TCL_ERROR )
	    return TCL_ERROR;
	  if( Tcl_ListObjAppendElement(interp,plist,objv[i + 1]) == TCL_ERROR )
	    return TCL_ERROR;
	}
	...
	LOG_MSG(interp,WRITE_LOG,__FILE__,__LINE__,
		...
		"error encrypting \"",Tcl_GetString(qStrList),"\"",NULL);
...
#define URLCFGEND (enum urlElement)QUERYSTRING+3
  ...
  if (res == TCL_CONTINUE) {
    ...
    if (paramGetIndexFromObj(interp, objv[1], params, "subcommand", 0, &opt) == TCL_ERROR)
      return TCL_ERROR;
	...
	if( ((urlData->urlformat) & urlElementFlags[i]) != 0 ) {
	  tmpres = Tcl_NewStringObj( &(urlElementOpts[i][1]),-1);

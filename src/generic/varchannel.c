 ...
 * close var channel
 * ------------------------------------------------------------------------- */
int varchannelCloseProc(ClientData clientData,
			Tcl_Interp *interp) {
...
int varchannelInputProc(ClientData clientData, 
  ...
  if( (clientData == NULL) || (buf == NULL) ) return -1;

  varChannel = (VarChannel *)clientData;
  if( varChannel->varName == NULL ) return -1;
  ...
  if( isNew ) varChannel->readCursor = 0;
...
int varchannelOutputProc(ClientData clientData, 
  ...
  var = Tcl_ObjSetVar2(varChannel->interp,
                       ...
                       NULL,
                       tmp,
                       TCL_GLOBAL_ONLY | TCL_APPEND_VALUE | TCL_LEAVE_ERR_MSG);
...
Tcl_Channel Web_GetChannelOrVarChannel(Tcl_Interp *interp, 
  ...
  if( (interp == NULL) || (name == NULL) || (mode == NULL) ) return NULL;
...
int Web_UnregisterVarChannel(Tcl_Interp *interp, char *name, 
  ...
  if( (interp == NULL) || (name == NULL) ) return TCL_ERROR;

  /* if name does not start with # we don't care */
  if( name[0] != '#' ) return TCL_OK;

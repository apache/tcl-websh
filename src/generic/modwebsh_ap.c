    ...
    if( webInterp->dtor == NULL ) {
      ...
      if (result == TCL_OK) {
        result = Tcl_ListObjReplace(interp,
                                    webInterp->dtor,length,0,1,&objv[1]);
...
int Web_Finalize(ClientData clientData, 
  ...
  if( webInterp == NULL ) return TCL_ERROR;
  if( webInterp->interp == NULL ) return TCL_ERROR;

  if( webInterp->dtor == NULL ) return TCL_OK;
  ...
  if (res == TCL_OK) {
    ...
    for( i = (len - 1); i >= 0; i-- ) {
      ...
      res = Tcl_ListObjIndex(webInterp->interp,webInterp->dtor,i,&tobj);
...
/*           fprintf(stderr,"DBG doFinalize -- PROBLEM: %s\n",Tcl_GetStringResult(webInterp->interp)); fflush(stderr); */
          ...
          LOG_MSG(webInterp->interp,WRITE_LOG | INTERP_ERRORINFO,
                  ...
                  "error evaluating \"",Tcl_GetString(tobj),"\"",NULL);
          continue;
        }
      }
    }
...
int Web_InterpCfg(ClientData clientData, 
  ...
  enum params {INTERP_REQUESTS, INTERP_START, INTERP_LASTUSED, INTERP_RETIRE};
  ...
  case INTERP_REQUESTS: {
    ...
    if (objc == 3)
      if (Tcl_GetLongFromObj(interp,objv[2],&(webInterp->numrequests)) != TCL_OK ) {
	return TCL_ERROR;
      }
  ...
  case INTERP_START: {
    ...
    if (objc == 3)
      if (Tcl_GetLongFromObj(interp,objv[2],&(webInterp->starttime)) != TCL_OK ) {
	return TCL_ERROR;
      }
  ...
  case INTERP_LASTUSED: {
    ...
    if (objc == 3)
      if (Tcl_GetLongFromObj(interp,objv[2],&(webInterp->lastusedtime)) != TCL_OK ) {
	return TCL_ERROR;
      }
...
int Web_InterpClassCfg(ClientData clientData, 
  ...
  enum params {CLASS_TTL, CLASS_IDLETIME, CLASS_REQUESTS};
  ...
  if (webInterpClass == NULL) {
    int isnew = 0;
    /* fixme: should get mtime for id, but this needs lock on mainInterp
       which would be an incremental lock ... */
  ...
  case CLASS_TTL: {
    ...
    if (objc == 4)
      if (Tcl_GetLongFromObj(interp,objv[3],&(webInterpClass->maxttl)) != TCL_OK ) {
	Tcl_MutexUnlock(&(conf->webshPoolLock));
	return TCL_ERROR;
      }
  ...
  case CLASS_IDLETIME: {
    ...
    if (objc == 4)
      if (Tcl_GetLongFromObj(interp,objv[3],&(webInterpClass->maxidletime)) != TCL_OK ) {
	Tcl_MutexUnlock(&(conf->webshPoolLock));
	return TCL_ERROR;
      }
  ...
  case CLASS_REQUESTS: {
    ...
    if (objc == 4)
      if (Tcl_GetLongFromObj(interp,objv[3],&(webInterpClass->maxrequests)) != TCL_OK ) {
	Tcl_MutexUnlock(&(conf->webshPoolLock));
	return TCL_ERROR;
      }
...

  {
    ...
    if (res == TCL_ERROR) {
      ...
      objPtr = Tcl_GetVar2Ex(conf->mainInterp, "errorInfo", NULL,
			     TCL_GLOBAL_ONLY);
      Tcl_SetVar2Ex(interp, "errorInfo", NULL, objPtr,
		    TCL_GLOBAL_ONLY);
      
      objPtr = Tcl_GetVar2Ex(conf->mainInterp, "errorCode", NULL,
			     TCL_GLOBAL_ONLY);
      Tcl_SetVar2Ex(interp, "errorCode", NULL, objPtr,
		    TCL_GLOBAL_ONLY);

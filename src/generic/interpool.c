 ...
 * createWebInterp
 * ------------------------------------------------------------------------- */
WebInterp* createWebInterp(websh_server_conf *conf, WebInterpClass *webInterpClass, char *filename, long mtime) {
  ...
  Tcl_CreateObjCommand(webInterp->interp, "web::interpclasscfg",
		       Web_InterpClassCfg, 
		       (ClientData)conf, NULL);

  /* ------------------------------------------------------------------------
   * rename exit !
   * --------------------------------------------------------------------- */
  code = Tcl_NewStringObj("rename exit exit.apache; proc exit {} {uplevel #0 return}", -1);
...
WebInterp *poolGetWebInterp(websh_server_conf *conf, char *filename, long mtime, request_rec *r) {
  ...
  if (strcmp(id, filename)) {
    ...
    if (Tcl_EvalObjEx(conf->mainInterp, fileCmd, 0) == TCL_OK) {
      ...
      Tcl_GetLongFromObj(conf->mainInterp, 
			 Tcl_GetObjResult(conf->mainInterp), 
			 &mtime);
    }
  ...
  if (entry != NULL) {
	...
	logToAp(webInterp->interp,NULL,"interpreter expired (source changed)");
	...
	if (webInterpClass->maxidletime && (t - webInterp->lastusedtime) > webInterpClass->maxidletime) {
	  logToAp(webInterp->interp,NULL,"interpreter expired (idle time reached)");
	  webInterp->state = WIP_EXPIRED;

	} else {
	  if (webInterpClass->maxttl && (t - webInterp->starttime) > webInterpClass->maxttl) {
	    logToAp(webInterp->interp,NULL,"interpreter expired (time to live reached)");
	    webInterp->state = WIP_EXPIRED;
	  } else {
	    found = webInterp;
	    break;
...
void poolReleaseWebInterp(WebInterp *webInterp) {
  ...
  if (webInterp != NULL) {
	...
	logToAp(webInterp->interp,NULL,"interpreter expired (request count reached)");
	webInterp->state = WIP_EXPIRED;
      }
    }
...
int initPool(websh_server_conf *conf) {
...
#else /* APACHE2 */
    ap_log_error(APLOG_MARK, APLOG_NOERRNO|APLOG_ERR, 0, conf->server, "could'nt create main interp");
#endif /* APACHE2 */
    return 0;
  }
  ...
  if (conf->scriptName != NULL ) {
    if (Tcl_EvalFile(conf->mainInterp, (char *)conf->scriptName) == TCL_ERROR) {
      errno = 0;
#ifndef APACHE2
      ap_log_printf(conf->server, Tcl_GetStringResult(conf->mainInterp));
#else /* APACHE2 */
      ap_log_error(APLOG_MARK, APLOG_NOERRNO|APLOG_ERR, 0, conf->server, Tcl_GetStringResult(conf->mainInterp));
#endif /* APACHE2 */
      return 0;
    }
    Tcl_ResetResult(conf->mainInterp);

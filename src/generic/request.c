...
int resetRequestData(Tcl_Interp *interp, RequestData *requestData) {
  ...
  if( (interp == NULL) || (requestData == NULL) ) return TCL_ERROR;

  if( removeTempFiles(interp,requestData) != TCL_OK ) return TCL_ERROR;
...
int removeTempFiles(Tcl_Interp *interp, RequestData *requestData) {
   ...
   * sanity
   * ----------------------------------------------------------------------- */
  if( requestData == NULL ) return TCL_ERROR;
  if( requestData->tmpFnList == NULL ) return TCL_ERROR;
  ...
  while( nextFromHashIterator(&iterator) != TCL_ERROR ) {
    ...
    if( tclo != NULL ) {
      LOG_MSG(interp,WRITE_LOG,__FILE__,__LINE__,
              ...
              "removing temporary file ",Tcl_GetString(tclo),".",NULL);
...
int Web_Param(ClientData clientData, 
  ...
  return paramGet((ParamList *)requestData->paramList,interp,objc,objv,0);
}
...
int Web_FormVar(ClientData clientData, 
  ...
  return paramGet((ParamList *)requestData->formVarList,interp,objc,objv, 0);
}
...
int Web_TempFile(ClientData clientData, 
  ...
  enum params {PATH,PREFIX,REMOVE};
  int         idx = -1;
...
Tcl_Obj *tempFileName(Tcl_Interp *interp, RequestData *requestData,
  ...
  if (path != NULL)   pathstring   = Tcl_GetString(path);
  if (prefix != NULL) prefixstring = Tcl_GetString(prefix);
  ...
  while( (appendToHashTable(requestData->tmpFnList,
			   Tcl_GetString(tclo),(void *)tclo) == TCL_ERROR) &&
	 (trycnt < 100) ) {
...
int Web_Request(ClientData clientData, 
  ...
  enum params {REQUESTRESET,DEFAULTCHANNELNAME};
  ...
  res = paramGet((ParamList *)requestData->request,
		 interp,objc,objv, 1);

...
int log_Init(Tcl_Interp *interp) {
  ...
  WebAssertData(interp,logtochannel,"log_Init/logtochannel plugin",TCL_ERROR)
...
char *createLogDestName(char *prefix, int cnt) {
  ...
  if( strlen(prefix) > 10) return NULL;
...
int destroyLogDest(void *dest, void *env) {
  ...
  if( (dest == NULL) || (env == NULL) ) return TCL_ERROR;
...
int Web_LogDest(ClientData clientData, 
  ...
  case ADD: { 
    ...
    iCurArg = argIndexOfFirstArg(objc-1,&(objv[1]),params,NULL) + 1;

    /* check for known options */
    iUnknown = argHasOnlyAccepted(objc-1,&(objv[1]),params,iCurArg-1);
    if( iUnknown > 0 ) {
      /* let Tcl format the error message */
      Tcl_GetIndexFromObj(interp,objv[iUnknown+1],params,"option", 0,&idx);
      return TCL_ERROR;
    }
    ...
    /* check for -maxchar */
    if( (tcloTmp = argValueOfKey(objc,objv,params[MAXCHAR])) != NULL ) {
      if( Tcl_GetLongFromObj(interp,tcloTmp,&maxCharInMsg) == TCL_ERROR ) {
	LOG_MSG(interp,WRITE_LOG | SET_RESULT,
                __FILE__,__LINE__,
		"web::logdest",WEBLOG_INFO,
		"cannot read long from -maxchar \"",
		Tcl_GetString(tcloTmp),"\"",
		NULL);
	return TCL_ERROR;
      }
    }
     ...
     * parse level
     * --------------------------------------------------------------------- */
    logLevel = parseLogLevel(interp,Tcl_GetString(objv[iCurArg]),"user",-1);
    if( logLevel == NULL ) {
      WebFreeIfNotNull(format);
      return TCL_ERROR;
    }
    ...
    WebAssertData(interp,name,
                  "cannot create name for log destination",TCL_ERROR);
     ...
     * and add to list
     * --------------------------------------------------------------------- */
    if( appendToHashTable(logData->listOfDests,name,(ClientData)logDest) == TCL_ERROR ) {
  ...
  case NAMES: {
	...
	else namesIsFirst = TCL_ERROR;
	logDest = (LogDest *)valueOfCurrent(&iterator);
        Tcl_AppendResult(interp,
			 keyOfCurrent(&iterator)," ",
			 logDest->filter->facility,".",
			 getSeverityName(logDest->filter->minSeverity),"-",
			 getSeverityName(logDest->filter->maxSeverity),
			 NULL);
      }
    }
    return TCL_OK;
  ...
  case DELETE: {
    ...
    switch (objc) {
    case 3: 
      logDest = (LogDest *)removeFromHashTable(logData->listOfDests,
...
int Web_LogFilter(ClientData clientData, 
  ...
  case ADD: {
    ...
    WebAssertData(interp,name,"cannot create name for log filter",TCL_ERROR);

    /* ------------------------------------------------------------------------
     * parse level
     * --------------------------------------------------------------------- */
    logLevel = parseLogLevel(interp,Tcl_GetString(objv[2]),"user",-1);
    if( logLevel == NULL ) {
      WebFreeIfNotNull(name);
      return TCL_ERROR;
    }
     ...
     * and add to list
     * --------------------------------------------------------------------- */
    if( appendToHashTable(logData->listOfFilters,name,(ClientData)logLevel) == \
        TCL_ERROR ) {
      Tcl_SetResult(interp,"cannot append \"",NULL);
      Tcl_AppendResult(interp,name,"\" to list",NULL);
      destroyLogLevel(logLevel,NULL);
  ...
  case NAMES: {
	...
	logLevel = (LogLevel *)valueOfCurrent(&iterator);
        Tcl_AppendResult(interp,keyOfCurrent(&iterator)," ",
			 ...
			 "-",
			 getSeverityName(logLevel->maxSeverity),NULL);
      }
    }
    return TCL_OK;
    break;
  ...
  case DELETE: {
    ...
    switch (objc) {
    case 3: 
      logLevel = (LogLevel *)removeFromHashTable(logData->listOfFilters,
                                                Tcl_GetString(objv[2]));
      if( logLevel == NULL ) {
...
int destroyLogPlugIn(void *plugIn, void *dum) {
  ...
  if( logPlugIn != NULL ) WebFreeIfNotNull(logPlugIn);
  return TCL_OK;
}
...
int registerLogPlugIn(Tcl_Interp *interp, char *type, LogPlugIn *logPlugIn) {
   ...
   * append
   * ----------------------------------------------------------------------- */
  if( logData->listOfPlugIns == NULL ) return TCL_ERROR;
  return appendToHashTable(logData->listOfPlugIns,type,(ClientData)logPlugIn);
}
...
void LOG_MSG(Tcl_Interp *interp, int flag, char *filename, int linenr, 
  ...
  if (interp == NULL) Tcl_AppendToObj(errobj,"interp = null",-1);
  ...
  if (flag & SET_RESULT) Tcl_AppendStringsToObj(resobj,msg,NULL);
  ...
  printf("%s\n",Tcl_GetString(errobj)); fflush(stdout);
#endif

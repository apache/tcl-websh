...
int Web_Send(ClientData clientData, 
   ...
   * arg check
   * --------------------------------------------------------------------- */
  WebAssertObjc((objc < 4) || (objc > 5), 1, "channel cmdnr string ??#?flags?");
 ...
 * receive_msg
 * ------------------------------------------------------------------------- */
int receive_msg(Tcl_Channel f,int *command,int *flags,int *size,void **data) {
  ...
  ret=Tcl_Read(f, (char *)&mh.version,sizeof(MsgHeader)-sizeof(u_long));
  if (ret==-1) { 
#ifdef MSGDEBUG
    printf("Error reading socket\n");
#endif

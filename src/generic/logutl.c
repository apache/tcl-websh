...
int webLog(Tcl_Interp *interp, char *levelStr, char *msg) {
  ...
  if( (levelStr == NULL) || (msg == NULL) || (interp == NULL)) 
    return TCL_ERROR;
  if( Tcl_InterpDeleted(interp) ) return TCL_ERROR;
...
int logImpl(Tcl_Interp *interp, LogData *logData, 
  ...
  if( logLevel == NULL ) return TCL_ERROR;
	  ...
	  if(logData->logSubst) {
	    ...
	    if( emsg == NULL ) {
	      ...
	      subst = Tcl_NewStringObj("subst",5);
              ...
              if( Tcl_ListObjAppendElement(NULL,subst,msg) != TCL_OK ) {
                ...
                res = Tcl_EvalObjEx(interp,subst,TCL_EVAL_DIRECT);
                Tcl_DecrRefCount(subst);
...
char *getSeverityName(Severity aSeverity) {
  ...
  if( aSeverity == invalid ) aSeverity = none;
...
LogLevel *parseLogLevel(Tcl_Interp *interp, 
  ...
  if ((fromlevel == invalid) || (tolevel == invalid)) {
    if (facility)
      Tcl_Free(facility);
    if (interp)
      Tcl_AppendResult(interp, "wrong log level \"", definition ,"\"", NULL);
    return NULL;
  }
  ...
  if( result) {
    ...
    if (facility)
      Tcl_Free(facility);
    if (interp)
      Tcl_AppendResult(interp, "cannot allocate memory for filter", NULL);
    return NULL;
  }
  /* return NULL; */
}
...
int doesPass(LogLevel *level, LogLevel *filter) {
  ...
  if ((level == NULL) || (filter == NULL) ) return TCL_ERROR;
...
int doesPassFilters(LogLevel *logLevel, Tcl_HashTable *hash) {
  ...
  if( (logLevel == NULL) || (hash == NULL) ) return TCL_ERROR;

  assignIteratorToHashTable(hash,&iterator);
  while( nextFromHashIterator(&iterator) != TCL_ERROR ) {

    filter = (LogLevel *)valueOfCurrent(&iterator);
    if( doesPass(logLevel,filter) == TCL_OK ) return TCL_OK;
  }
    ...
    if (*c == '$') {
	  ...
	  if( maxCharInMsg == -1 ) {
        ...
        case 'n':
          ...
          break;
        case 'l':
          Tcl_AppendToObj(fmsg, getSeverityName(level->minSeverity), -1);
          break;
        case 'f':

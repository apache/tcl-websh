...
void destroyNcaD(ClientData clientData, Tcl_Interp *interp) {
  ...
  if( clientData != NULL ) Tcl_DecrRefCount((Tcl_Obj *)clientData);
}
...
int Web_DecryptD(ClientData clientData, 
  ...
  LOG_MSG(interp,WRITE_LOG,
	  __FILE__,__LINE__,
	  "web::decryptd",WEBLOG_DEBUG,
	  "crypt type not recognized",NULL);
...
Tcl_Obj* encryptNcaD(Tcl_Interp *interp, ClientData clientData, Tcl_Obj *in) {
  ...
  if( (clientData == NULL) || (in == NULL) ) return NULL;
  key = (Tcl_Obj *)clientData;
  ...
  if( Tcl_GetCharLength(in) < 1 ) return Tcl_NewObj();

  if( crcAdd(in) != TCL_OK ) return NULL;
...
Tcl_Obj *decryptNcaD(Tcl_Obj *key, Tcl_Obj *in) {
   ...
   * sanity
   * ----------------------------------------------------------------------- */
  if( (key == NULL) || (in == NULL) ) return NULL;

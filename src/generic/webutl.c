...
int handleConfig(Tcl_Interp *interp, Tcl_Obj **tclo, Tcl_Obj *newValue, int deleteIfEmpty) {
   ...
   * get
   * ----------------------------------------------------------------------- */
  if (newValue == NULL) return TCL_OK;
...
Tcl_Obj *tclSetEnv(Tcl_Interp *interp, char *key, Tcl_Obj *val) {
  ...
  if( (interp == NULL) || (key == NULL) ) return NULL;
  ...
  if( val == NULL ) {
    ...
    if( tmp != NULL ) res = Tcl_DuplicateObj(tmp);

  } else {

    /* set */
    res = Tcl_ObjSetVar2(interp,arrayName,keyObj,val, TCL_LEAVE_ERR_MSG);
  }
...
Tcl_Obj *Web_GetOrCreateGlobalVar(Tcl_Interp *interp, Tcl_Obj *name, 
  ...
  if( Tcl_InterpDeleted(interp) ) return NULL;
